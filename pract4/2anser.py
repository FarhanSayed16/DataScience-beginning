...
def calculate_attacks(board):
    """Calculate number of attacking queen pairs on the board."""
    n = len(board)
    attacks = 0
    for i in range(n):
...
def get_best_neighbor(board):
    """Generate neighbors and return the one with minimum attacks."""
    n = len(board)
    best_board = board[:]
...
def hill_climbing(n, max_restarts=100):
    ...
    for restart in range(max_restarts):
        ...
        while True:
            ...
            if neighbor_attacks >= current_attacks:  # No better neighbor
                break
